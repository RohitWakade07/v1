...
const routeLabels: Record<string, string> = {
  ...
  proof: 'Proof', submit: 'Submit', results: 'Results', profile: 'Profile',
}

const useBreadcrumbs = () => {
  const location = useLocation()
  const segments = location.pathname.split('/').filter(Boolean)
  if (segments.length <= 1) return [{ label: 'Dashboard', to: '/student' }]
  // skip the 'student' prefix segment
  const rest = segments.slice(1)
  return rest.map((seg, i) => ({
    label: routeLabels[seg] ?? (seg.length === 36 ? `${seg.slice(0, 8)}…` : seg),
    to: '/student/' + rest.slice(0, i + 1).join('/'),
  }))
}
