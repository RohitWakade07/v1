...
export const SubmissionsPage = () => {
  ...
  const rows = useMemo(() => {
    return filteredData.map((s) => [
      ...
      <span key="type" className="font-mono text-xs uppercase text-text-secondary">
        {s.source_type}
      </span>,
      <span key="attempt" className="font-mono text-xs text-text-primary">
        #{s.attempt_number}
      </span>,
      <StatusBadge key="status" status={s.status} />,
      <span key="score" className="font-mono text-sm font-medium">
        {s.score != null ? (
      ...
      <span key="submitted" className="text-xs text-text-secondary">
  ...
  return (
    <PageWrapper>
      <PageHeader
        ...
        description={`${submissions?.length ?? 0} total submissions · ${pendingCount} in progress`}
      />
