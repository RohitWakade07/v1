...
class ProofService:
    ...
    async def submit_proof(
        ...
        score_breakdown_str = json.dumps({
            test_id: {
                "passed": r.passed,
                "score": r.score,
            }
        ...
        # ── 9.1 Generate FinalResult ──────────────────────────────────
        passed = final_score >= (assignment.max_score * 0.4)
        ...
        # ── 9.2 Generate Certificate if passed ────────────────────────
        if passed:
            cert_code = f"CERT-{assignment.slug.upper()}-{current_student.roll_number.upper()}-{uuid.uuid4().hex[:5].upper()}"
