...
const ToastItem = ({
  ...
  return (
    <div
      className={cn(
        'animate-slide-in-right relative overflow-hidden rounded-xl border bg-navy-900 px-4 py-3 shadow-card',
        styles[type],
      )}
      role="alert"
      aria-live="assertive"
    >
      ...
      <div className="absolute bottom-0 left-0 h-0.5 w-full bg-navy-800">
        <div
          ref={barRef}
          className={cn('h-full', type === 'success' ? 'bg-accent-teal' : type === 'error' ? 'bg-status-danger' : type === 'warning' ? 'bg-status-warning' : 'bg-accent-blue')}
          style={{ width: '100%' }}
        />
      </div>
    </div>
